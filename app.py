...
from password_utils import validate_password, hash_password
from dotenv import load_dotenv

load_dotenv()
SENDGRID_API_KEY = os.getenv("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")

def send_reset_email(email, reset_url):
    message = Mail(
        from_email='aaluoza2406@gmail.com',
        to_emails=email,
        subject='Reset Your Password - Household Services',
...
app.config['SECRET_KEY'] = '7e05747e25717dde612edd9b60b9d2cf'  # Needed for sessions
...
def register():
    ...
    if form.validate_on_submit():
        ...
        password_errors = validate_password(form.password.data, form.username.data)
        if password_errors:
...
def professional_dashboard():
    ...
    responses:
      200:
        description: Renders the professional dashboard page with active service requests.
      302:
    ...
    # Get active service requests for this professional
    service_requests = ServiceRequest.query.filter_by(
        professional_id=professional_id
    ).all()
    
    # Get completed service requests for this professional
    completed_requests = ServiceRequest.query.filter(
        ServiceRequest.professional_id == professional_id,
        ServiceRequest.service_status == 'completed'
    ).all()
    
    # Get professional's profile info
    ...
    # Create dictionaries for customer and service info
    cust_dict = {}
    service_dict = {}
    ...
    for request in service_requests + completed_requests:
        if request.customer_id not in cust_dict:
            cust_dict[request.customer_id] = CustomerProfile.query.filter_by(
...
def forgot_password():
    ...
    if form.validate_on_submit():
        username = form.email.data  # Treating email field as username
        user = User.query.filter_by(username=username).first()

        if user:
            flash("User found! Enter your new password.", "success")
            return redirect(url_for('reset_password_page', username=username))  # Direct reset
        else:
...
def reset_password_page(username):
    ...
    if form.validate_on_submit() and user:
        password_errors = validate_password(form.password.data, username)
        if password_errors:
            for error in password_errors:
                flash(error, 'danger')
...
def refresh_session():
    ...
    return '', 204  # Empty response, useful for fetch
